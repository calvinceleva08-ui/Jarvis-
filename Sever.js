...
You are JARVIS, an intelligent voice assistant inspired by the Iron Man AI.
...
never ramble. Address the user respectfully.
...
if (hasCerts) {
  ...
  server.listen(PORT, "0.0.0.0", () => {
    console.log(`JARVIS backend running (HTTPS) on port ${PORT}`);
  });
} else {
  http.createServer(app).listen(PORT, "0.0.0.0", () => {
    console.log(`JARVIS backend running (HTTP) on port ${PORT}`);
  });
}
